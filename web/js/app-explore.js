...
(function ($) {
    ...
    for (var i = 0; i < projects.length; i++) {
        if (projects[i]['min_price'] <= "1.00") {
            project_price = 'Consult Us for Price';
        }
        else {
            project_price = '$ ' + Math.round(projects[i]['min_price']).toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + '+';
        }
        var address_url = projects[i]['address'].replace(/ /g, "-");
        address_url = address_url.replace(/,/g, "");
        props.push(
            {
                id: projects[i]['id'],
                title: projects[i]['title'],
                image: projects[i]['front_image'],
                type: "On Sale",
                price: project_price,
                address: projects[i]['address'],
                address_url: address_url,
                position: {
                    lat: projects[i]['latitude'],
                    lng: projects[i]['longitude']
                },
    ...
    var addMarkers = function (props, map) {
        $.each(props, function (i, prop) {
            ...
            var infoboxContent = '<div class="infoW">' +
                ...
                '<a href="../projects/' + prop.id + '" class="btn btn-sm btn-round btn-green viewInfo">View</a>' +
                '</div>' +
                '</div>';
    ...
    setTimeout(function () {
        ...
        var map_lat = locations['latitude'];
        var map_lng = locations['longitude'];
        var zoom_level = locations['level'];
        map.setCenter(new google.maps.LatLng(map_lat, map_lng));
        map.setZoom(zoom_level);
        //map.setCenter(new google.maps.LatLng(-37.813611, 144.963056));
        //map.setZoom(14);
        if ($('#address').length > 0) {
            newMarker = new google.maps.Marker({
                position: new google.maps.LatLng(-37.814317, 144.963138),
                map: map,
                icon: new google.maps.MarkerImage(
